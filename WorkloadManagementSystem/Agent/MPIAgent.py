...
import os, sys, re, string, time, commands, random
...
class MPIAgent(AgentModule):
  ...
  def execute(self):
    ...
    while self.status <> 'Exit':
     if  self.masterFlag == False and self.slaveFlag == False:
       ...
       tempDir = "/tmp"
       tempDirNum = (str(self.jobID))
       dirTMP = os.path.join(tempDir, tempDirNum)
       print dirTMP
       os.environ['TMP_DIR_PARROT'] = dirTMP
 
       if self.masterFlag == True:
         cmd = ('chirp_server -X -o chirp_log.txt -d all -a globus -a unix -a hostname -r %s &') % dirTMP
         x = commands.getstatusoutput(cmd)
         print "============================================================================================="
         print x 
         print "============================================================================================="
  ...
  def __finish(self, message):
    ...
    cmd =('rm -rf $TMP_DIR_PARROT') 
    status, result = commands.getstatusoutput(cmd)
    if status==0:
      self.log.debug('Parrot path removed')
    else:
      self.log.error('Failed to remove parrot path')
    self.jobPath = False
    self.sharedHome = False
    return S_ERROR('Message')
  ...
  def testStatusRing(self, directory):
    ...
    if self.mpiFlavor == 'MPICH2':
      ...
      cmd = ('sleep 20; parrot_run -m mountfile  mpdtrace -l|wc -l')
...
#################################################################################
  ...
  def masterStart(self):
    ...
    if self.mpiFlavor == 'MPICH2':
      ...
      if dir == True:
        ######updDict = {'JobID': self.jobID, 'RingID': self.ringID, 'Status': self.status}
        updDict = {'JobID': self.jobID, 'RingID': self.ringID, 'Status': 'Failed'}
        result = self.mpiService.setRingStatus(updDict)
        if not result['OK']:
      ...
      if status == 0:
        ...
        Message = ('Failed to find the MPDTRACE command ???')  
        res = self.__rescheduleAndFinalize(self.jobID, self.ringID, self.status, Message)
        if not res['OK']:
             self.log.error('Failed to reschedule the job and finalize the process')
      ...
      parrotCMD =  (commands.getoutput('which parrot_run'))
 
      ####### MOUNTFILE

      status, result = commands.getstatusoutput("mkdir $TMP_DIR")
      applicationDirectory = os.path.join(localPath,'Application')
      cmd = ('mkdir %s') % (applicationDirectory)
      status, result = commands.getstatusoutput(cmd)

      dirTMP = os.environ.get['TMP_DIR']
      mountInputFile = ('%s /chirp/%s ') % (dirTMP, self.master)
      path = os.environ('$PATH')
      mountApplicationFile = ('%s/Application /chirp/dirac.eela.if.ufrj.br ') % (path)
      localPath = `os.environ.get("HOME")`
      mountfile = os.path.join(localPath, 'mountfile')
      f = open(mountfile, 'w')
      f.write(mountInputFile)
      f.write(mountApplicationFile)
      f.close()

      status, result = commands.getstatusoutput("echo $TMP_DIR_PARROT")
      tmpDIRParrot = result
      parrot_env = os.environ
      status, result = commands.getstatusoutput("mkdir $TMP_DIR_PARROT")
      start = Popen(["%s" % parrotCMD, '-a', 'globus', '-m', "mountfile", "%s" % result1, "--ifhn=%s" % self.master], executable=parrotCMD, env=parrot_env).pid
      os.system('sleep 5')
      cmd = ("""mpdtrace -l |cut -d_ -f2 |cut -d" " -f1""")
      status, result = commands.getstatusoutput("""parrot_run -m mountfile  mpdtrace -l |cut -d_ -f2 |cut -d" " -f1""")
      self.log.debug(result)
      resultado = int(result) 
      print "mpdtrace 1"
      print result
      if status == 0 and type(resultado) == int:
...
#############################################################################
  ...
  def slaveStart(self, slaveDict):
    ...
    if self.mpiFlavor=="MPICH2":
      ...
      dirTMP = os.environ.get['TMP_DIR']
      mountInputFile = ('%s /chirp/%s ') % (dirTMP, master)
      path = os.environ('$PATH')
      mountApplicationFile = ('%s/Application /chirp/dirac.eela.if.ufrj.br ') % (path)
      localPath = `os.environ.get("HOME")`
      mountfile = os.path.join(localPath, 'mountfile')
      ...
      parrotCMD = (commands.getoutput('which parrot_run'))
      cmd = ('%s -a globus -m mountfile %s -h %s -p %s -n &') % (parrotCMD, master, port)
