...
#################################################################################
  def __match(self, matchDict, statusDict):
    ...
    if status == 'Empty':
       ...
       print "--------------------------------------"
       # VANESSA
       if result['Value']['MPIFlavor'] == '' or result['Value']['NumProc'] == '':
         gLogger.info("MPI information Flavor or Num Proc is missing in the Job JDL, job is going to be rescheduled")
         slock.release()
         jobID = result['Value']['JobID']
         rescheduleJobs = jobDB.rescheduleJob(jobID)
         return S_ERROR(['Failed, MPI Information is missing'])        
...
##############################################################################
  ...
  def export_getJobOptParameters (self, jobID):
    ...
    print "matcherParams: ", matcherParams
...
################################################################################
  ...
  def matchJob(self, resourceJDL):
    ...
    """
    resJDL = ClassAd(resourceJDL)
    self.OwnerDN = resJDL.getAttributeString('OwnerDN')
    self.OwnerGroup = resJDL.getAttributeString('OwnerGroup')
     
    print "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
    print  self.OwnerDN,  self.OwnerGroup
    print "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
 
    matcher = RPCClient('WorkloadManagement/Matcher', timeout = 600, delegatedDN=self.OwnerDN, delegatedGroup=self.OwnerGroup)
...
#############################################################################
  ...
  def __prepareSecurityDetails(self):
    ...
    print "PREPARE SECURITY", ownerDN
